...
void JNuBeamFlux::Reset() {
  ...
  for (int i = 0; i < 2; i++) {
    NuXnu[i] = NuBpos[i] = NuBtilt[i] = NuBrms[i] = NuEmit[i] = NuAlpha[i] =
        -999999.;
    //      NuRseed[i] = -999999;
  }
  for (int i = 0; i < 3; i++) NuGpos0[i] = NuGvec0[i] = NuHcur[i] = -999999.;
  for (int i = 0; i < 4; i++)
    NuParentDecP4[i] = NuParentDecX4[i] = NuParentProP4[i] = NuParentProX4[i] =
        -999999.;
  NuNg = -1;
  for (int i = 0; i < kNgmax; i++) {
    NuGcosbm[i] = NuGdistc[i] = NuGdistal[i] = NuGdistti[i] = NuGdistfe[i] =
        -999999.;
    NuGpid[i] = 0;
